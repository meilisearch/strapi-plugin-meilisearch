...
export const meilisearchConnectorService = ({ strapi, adapter, config }) => {
  ...
  return {
     ...
     */
    deleteEntriesFromMeiliSearch: async ({ contentType, entriesId }) => {
      const { apiKey, host } = await store.getCredentials()
      const client = meilisearchClient({ apiKey, host })
     ...
     */
    updateEntriesInMeilisearch: async ({ contentType, entries }) => {
      const { apiKey, host } = await store.getCredentials()
      const client = meilisearchClient({ apiKey, host })
     ...
     */
    addEntriesToMeilisearch: async ({ contentType, entries }) => {
      const { apiKey, host } = await store.getCredentials()
      const client = meilisearchClient({ apiKey, host })
     ...
     */
    addContentTypeInMeiliSearch: async ({ contentType }) => {
      const { apiKey, host } = await store.getCredentials()
      const client = meilisearchClient({ apiKey, host })
      const indexUids = config.getIndexNamesOfContentType({ contentType })
     ...
     */
    getContentTypesWithSameIndex: async ({ contentType }) => {
      const indexUids = config.getIndexNamesOfContentType({ contentType })
