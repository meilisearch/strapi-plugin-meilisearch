import { isObject } from "src/utils/type-checking"
...
export const meilisearchConfigService = ({ strapi }) => {
  ...
  return {
     ...
     */
    transformEntries: async ({ contentType, entries = [] }) => {
      const collection = contentTypeService.getCollectionName({ contentType })
      const contentTypeConfig = meilisearchConfig[collection] || {}
     ...
     * @return {Promise<Array<Object>>} - Converted or mapped data
     */
    filterEntries: async ({ contentType, entries = [] }) => {
      const collection = contentTypeService.getCollectionName({ contentType })
      const contentTypeConfig = meilisearchConfig[collection] || {}
     ...
     */
    listContentTypesWithCustomIndexName: ({ indexName }) => {
      const contentTypes =
        strapi
          .plugin('meilisearch')
          .service('contentType')
     ...
     */
    removeSensitiveFields: ({ contentType, entries }) => {
      const collection = contentTypeService.getCollectionName({ contentType })
      const contentTypeConfig = meilisearchConfig[collection] || {}
     ...
     */
    removeUnpublishedArticles: ({ entries, contentType }) => {
      const collection = contentTypeService.getCollectionName({ contentType })
      const contentTypeConfig = meilisearchConfig[collection] || {}
     ...
     * @return {Array<Object>} - Published entries.
     */
    removeLocaleEntries: ({ entries, contentType }) => {
      const collection = contentTypeService.getCollectionName({ contentType })
      const contentTypeConfig = meilisearchConfig[collection] || {}
