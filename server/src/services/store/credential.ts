const storeCredentialService =  ({ store, strapi }) => ({
  /**
   ...
   */
  getApiKeyIsFromConfigFile: async () => {
   ...
   * @returns {Promise<boolean>} APIKeyCameFromConfigFile
   */
  setApiKeyIsFromConfigFile: async (value) => {
   ...
   * @returns {Promise<boolean>} HostCameFromConfigFile
   */
  setHostIsFromConfigFile: async (value) => {
    return store.setStoreKey({ key: 'meilisearch_host_config', value })
  },
})

export default storeCredentialService;