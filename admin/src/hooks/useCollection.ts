...
export function useCollection() {
  ...
  const fetchCollections = async () => {
    const { data, error } = await get(`/${PLUGIN_ID}/content-type/`)
  ...
  const deleteCollection = async ({ contentType }) => {
    try {
      const { error } = await del(`/${PLUGIN_ID}/content-type/${contentType}`)
      if (error) {
        handleNotification({
          type: 'warning',
          message: error.message,
  ...
  const addCollection = async ({ contentType }) => {
    try {
      const { error } = await post(`/${PLUGIN_ID}/content-type`, {
        body: {
          contentType,
        },
      })
  ...
  const updateCollection = async ({ contentType }) => {
    try {
      const { error } = await put(`/${PLUGIN_ID}/content-type`, {
        body: {
          contentType,
        },
      })
